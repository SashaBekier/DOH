...
public class ProfileView extends DAHView{
	...
	private void drawMiddle() {
		...
		ValidatedTextField firstNameF = new ValidatedTextField(
				s-> Validators.hasContent(s),updateUser);
		...
		ValidatedTextField lastNameF = new ValidatedTextField(
				s -> Validators.hasContent(s),updateUser);
		...
		Image vipImage = activeUser.hasVIP()? new Image("assets/vipActive.png"):
			new Image("assets/vipAvailable.png");
		Button vipButton = activeUser.hasVIP()? new Button("Unsubscribe"):
		...
		layout.getChildren().addAll(DAHStyles.verticalSpacer(20),
				greeting, mainpanel);
